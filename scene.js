...
class TestScene {
    constructor() {
        ...
        this.doorPositions = {
            north: 8, // Position along the north wall (0-gridWidth)
    ...
    drawWalls(ctx) {
        ...
        for (let i = 0; i < this.gridWidth; i++) {
            ...
            for (let row = 1; row < wallHeight; row++) {
                const verticalOffset = -row * this.cellHeight * 0.7; // Stack tiles with some overlap
    ...
    drawWestWallRow(ctx, position, verticalOffset, currentScene, row) {
        ...
        // For doors, only render them on the bottom row (row 0)
        // and render taller doors that span both rows
        if (position === this.doorPositions.west && row === 0) {
            // DIRECT DEBUG: First check if the door is open by querying doorwayManager directly
            const doorways = doorwayManager.doorwaysByScene[currentScene] || [];
            const matchingDoors = doorways.filter(door => 
                door.isWallDoorway && 
            ...
            if (matchingDoors.length > 0) {
                ...
                console.log(`DIRECT DOOR CHECK: Found west door at pos ${position}, isOpen=${isOpen}`);
                ...
                console.log(`DIRECT DOOR CHECK: No west door found at pos ${position}`);
                ...
                // If no door was rendered by the manager, use the fallback
                if (!doorRendered) {
                    // Use isometricRenderer to draw the northwest door at double height
                    isometricRenderer.renderDoorway(
                        ctx, 
                        screenX, 
                        screenY, 
        ...
        // Skip rendering the wall tile behind the door on the second row
        else if (position === this.doorPositions.west && row > 0) {
            // Skip rendering wall tile here (door spans this space)
        }
        else {
            // Regular wall tile
    ...
    drawRoomLabels(ctx) {
        try {
            ...
            const circuitGridX = -18.9;    // Exact X position from the screenshot
            const circuitGridY = -15.4;   // Positioned above the north door
            ...
            if (circuitPos) {
                ...
                // Draw the animated arrow pointing to the Vibeverse Arcade door
                this.drawVibeverseDoorArrow(ctx);
            }
            
            // Draw Neon Phylactery label (east door) - keeping it aligned with the eastern doorway
            // Adjust coordinates to maintain consistency with Circuit Sanctum positioning
            const neonGridX = -29.2;    // Horizontal position at east door
            const neonGridY = -9.5;     // Vertical position of east door
    ...
    drawComingSoonLabels(ctx) {
        try {
            ...
            // NOTE: Circuit Sanctum/Vibeverse Arcade no longer shows COMING SOON label
            // as it's now fully implemented
    ...
    drawVibeverseDoorArrow(ctx) {
        try {
            ...
            const arrowGridX = -19.4;  // Match the Vibeverse Arcade label X-position
            const arrowGridY = -17;  // Place above the label
